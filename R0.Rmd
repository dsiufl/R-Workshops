...
The goal of this workshop, and the next 2 R workshops, is to get you all from 'man I have no idea how to do that' to 'yeah I think I could figure out how to do this.'
...
```{r}
#Empty dataframe
students<-data.frame(studentID=factor(),
              age=numeric(),
              university=factor(),
              hired=factor()
              )
...
Here we will introduce some common checks using logicals


```{r}
x<pi

##are any values in x less than pi
sum(x<pi) >  0

##are all values greater than -1
sum(x< -1) > 0

#picking specific values by subsetting with logical vectors
x[x<pi]
```
